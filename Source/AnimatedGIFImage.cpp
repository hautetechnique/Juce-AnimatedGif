...
{
public:
    AnimatedGIFLoader (InputStream& in)
    : input (in),
    ...
    firstcode (0), oldcode (0), clearCode (0), endCode (0)
    {
        ...
        for (;;)
        {
            ...
            if (buf[0] == '!')
            {
                if (readExtension (transparent, frameDuration))
                    continue;
            ...
            if (input.read (buf, 9) == 9)
            {
                ...
                image.add(Image (transparent >= 0 ? Image::ARGB : Image::RGB,
                                 imageWidth, imageHeight, transparent >= 0));
                
                image.setFrameDuration(image.size() - 1, frameDuration);
                
                image.getLast().getProperties()->set ("originalImageHadAlpha", transparent >= 0);
...
private:
    ...
    int readExtension (int& transparent, int& frameDuration)
    {
        uint8 type;
    ...
    bool readImage (const int interlace, const int transparent)
    {
        ...
        const Image::BitmapData destData (image.getLast(), Image::BitmapData::writeOnly);
        uint8* p = destData.getPixelPointer (0, 0);
        const bool hasAlpha = image.getLast().hasAlphaChannel();
